   ...
   X509CertificateInfo *certInfo)
{
   ...
   if(error) return __ERROR_BAD_CERTIFICATE;
   ...
   *totalLength = tag.totalLength;
   ...
   while(length > 0)
   {
      ...
      else if(critical)
      {
         ...
         return __ERROR_UNSUPPORTED_EXTENSION;
      }
   }
   ...
   const X509CertificateInfo *issuerCertInfo)
{
   ...
   if(currentTime != 0)
   {
      ...
      if(currentTime < notBefore || currentTime > notAfter)
         return __ERROR_CERTIFICATE_EXPIRED;
   }
   ...
   else
   {
      ...
      return __ERROR_UNSUPPORTED_SIGNATURE_ALGO;
   }
   ...
   else if(ecdsaSignAlgo)
   {
      ...
      error = (curveInfo == NULL) ? __ERROR_BAD_CERTIFICATE : __NO_ERROR;
