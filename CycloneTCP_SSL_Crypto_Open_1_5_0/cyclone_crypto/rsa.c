...
error_t rsadp(const RsaPrivateKey *key, const Mpi *c, Mpi *m)
{
   ...
   {
      //Report an error
      error = __ERROR_INVALID_PARAMETER;
   }
   ...
   size_t ciphertextLength, uint8_t *message, size_t messageSize, size_t *messageLength)
{
   ...
   do
   {
      ...
      {
         //Report an error
         error = __ERROR_UNEXPECTED_VALUE;
         break;
      }
   ...
   const uint8_t *digest, const uint8_t *signature, size_t signatureLength)
{
   ...
   do
   {
      ...
      //Assume an error...
      error = __ERROR_INVALID_SIGNATURE_ALGO;
      ...
      error = memcmp(digest, d, dLength) ? __ERROR_INVALID_SIGNATURE : __NO_ERROR;
   ...
   size_t *oidLength, const uint8_t **digest, size_t *digestLength)
{
   ...
   if(emLength < 11) return __ERROR_INVALID_LENGTH;

   //The first octet of EM must have a value of 0x00
   if(em[0] != 0x00) return __ERROR_UNEXPECTED_VALUE;
   //The block type BT shall be 0x01
   if(em[1] != 0x01) return __ERROR_UNEXPECTED_VALUE;
   ...
   for(i = 2; i < emLength; i++)
   {
      ...
      if(em[i] != 0xFF) return __ERROR_INVALID_PADDING;
   }

   //Check whether the padding string is properly terminated
   if(i >= emLength) return __ERROR_INVALID_PADDING;
   //The length of PS cannot be less than 8 octets
   if(i < 10) return __ERROR_INVALID_PADDING;
