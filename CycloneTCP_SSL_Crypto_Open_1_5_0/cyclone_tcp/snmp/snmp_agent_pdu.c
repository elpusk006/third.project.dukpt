   ...
   for(index = 1; length > 0; index++)
   {
      ...
      else
      {
         ...
         else
         {
            ...
            else if(error == __ERROR_OBJECT_NOT_FOUND)
            {
      ...
      if(error)
      {
         ...
         {
            //Catch exception
            if(error == __ERROR_ACCESS_DENIED ||
               error == __ERROR_OBJECT_NOT_FOUND)
            {
            ...
            else if(error == __ERROR_INSTANCE_NOT_FOUND)
            {
...
error_t snmpParseGetBulkRequestPdu(SnmpAgentContext *context, const uint8_t *p, size_t length)
{
   ...
   //Check status code
   if(error == __ERROR_BUFFER_OVERFLOW)
   {
