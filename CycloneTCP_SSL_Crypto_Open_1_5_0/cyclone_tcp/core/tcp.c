...
error_t tcpConnect(Socket *socket)
{
   ...
   if(socket->state != TCP_STATE_CLOSED)
      return __ERROR_ALREADY_CONNECTED;
   ...
   //Failed to establish connection?
      ...
      return __ERROR_CONNECTION_FAILED;
   //Timeout exception?
   else
   ...
   while(1)
   {
      ...
      if(newSocket != NULL)
      {
         ...
         if(!error)
         {
            ...
            //Initialize TCP control block
            newSocket->iss = tcpIpStackGetRand();
   ...
   size_t length, size_t *written, uint_t flags)
{
   ...
   for(totalLength = 0; totalLength < length; )
   {
      ...
      case TCP_STATE_TIME_WAIT:
         ...
         return __ERROR_CONNECTION_CLOSING;

      //CLOSED state?
      default:
         //The connection was reset by remote side?
         return (socket->resetFlag) ? __ERROR_CONNECTION_RESET : __ERROR_NOT_CONNECTED;
      }
   ...
   *received = 0;
   ...
   while(*received < size)
   {
      ...
      default:
         //The connection was reset by remote side?
         if(socket->resetFlag)
            return __ERROR_CONNECTION_RESET;
