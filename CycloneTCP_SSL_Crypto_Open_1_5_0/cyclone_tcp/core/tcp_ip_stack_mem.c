...
error_t chunkedBufferSetLength(ChunkedBuffer *buffer, size_t length)
{
   ...
   else
   {
      ...
      while(i < buffer->maxChunkCount && length > 0)
      {
         ...
         //Failed to allocate memory?
         if(!chunk->address) return __ERROR_OUT_OF_MEMORY;
   ...
   //Return status code
   return (length > 0) ? __ERROR_OUT_OF_RESOURCES : __NO_ERROR;
}
   ...
   const ChunkedBuffer *src, size_t srcOffset, size_t length)
{
   ...
   //Return status code
   return (length > 0) ? __ERROR_FAILURE : __NO_ERROR;
}
