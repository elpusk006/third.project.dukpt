   ...
   uint32_t ackNum, size_t length, bool_t addToQueue)
{
   ...
   if(!buffer) return __ERROR_OUT_OF_MEMORY;
 ...
 * @param[in] length Length of the segment data
 * @return __NO_ERROR if the incoming segment is acceptable, __ERROR_FAILURE otherwise
 **/
 ...
 * @param[in] segment Pointer to the TCP segment to check
 * @param[in] length Length of the segment data
 * @return __ERROR_FAILURE if the SYN is in the window, __NO_ERROR otherwise
 **/
 ...
 * @param[in] segment Pointer to the TCP segment to check
 * @param[in] length Length of the segment data
 * @return __NO_ERROR if the acknowledgment is acceptable, __ERROR_FAILURE otherwise
 **/
...
error_t tcpNagleAlgo(Socket *socket)
{
   ...
   if((int_t) u < 0) return __NO_ERROR;
