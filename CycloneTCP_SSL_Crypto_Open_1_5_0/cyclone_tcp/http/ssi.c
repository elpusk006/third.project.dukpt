...
error_t ssiExecuteScript(HttpConnection *connection, const char_t *uri, uint_t level)
{
   ...
   while(1)
   {
      ...
      //Partial match?
      else if(error == __ERROR_PARTIAL_MATCH)
      {
         //Send the part of the file that precedes the tag
         error = httpWriteStream(connection, buffer + pos, n);
         //Failed to send data?
   ...
   const char_t *tag, size_t length, const char_t *uri, uint_t level)
{
   ...
   else
   {
      ...
      error = __ERROR_INVALID_TAG;
   }

   //Invalid SSI directive?
   if(error == __ERROR_INVALID_TAG)
   {
   ...
   const char_t *tag, size_t length, const char_t *uri, uint_t level)
{
   ...
   *separator = '\0';
   ...
   if(!strcasecmp(attribute, "file"))
   {
      ...
      //Failed to allocate memory?
      if(!path) return __ERROR_OUT_OF_MEMORY;
 ...
 * @retval __NO_ERROR if the specified tag has been found
 * @retval __ERROR_PARTIAL_MATCH if a partial match occurs
 * @retval __ERROR_NO_MATCH if the tag does not appear in the string
 **/
   ...
   for(i = 0; i <= sLen; i++)
   {
      ...
      else if((i + j) == sLen && j > 0)
      {
         ...
         *pos = i;
         ...
         return __ERROR_PARTIAL_MATCH;
      }
   }

   //The tag does not appear in the string
   return __ERROR_NO_MATCH;
}
