...
error_t httpServerInit(HttpServerContext *context, const HttpServerSettings *settings)
{
   ...
   do
   {
      ...
      if(!osCreateSemaphore(&context->semaphore, HTTP_SERVER_MAX_CONNECTIONS))
      {
   ...
   if(error)
   {
      ...
      osDeleteSemaphore(&context->semaphore);
      //Close socket
      socketClose(context->socket);

#if (HTTP_SERVER_DIGEST_AUTH_SUPPORT == ENABLED)
      //Delete mutex
      osDeleteMutex(&context->nonceCacheMutex);
#endif
   }
...
void httpListenerTask(void *param)
{
   ...
   for(counter = 1; ; counter++)
   {
      ...
      if(!osWaitForSemaphore(&context->semaphore, 0))
      {
         //Debug message
      ...
      connection = osAllocMem(sizeof(HttpConnection));
      ...
      if(connection)
      {
         ...
         if(!error)
         {
            ...
            if(task == OS_INVALID_HANDLE)
            {
               ...
               //Release semaphore
               osReleaseSemaphore(connection->semaphore);
               //Free previously allocated memory
               osFreeMem(connection);
            }
         }
      }
   }
...
void httpConnectionTask(void *param)
{
   ...
   if(!error)
   {
      ...
      for(counter = 0; counter < HTTP_SERVER_MAX_REQUESTS; counter++)
      {
         ...
         //Bad request?
         if(error == __ERROR_INVALID_REQUEST)
         {
         ...
         else if(error == __ERROR_AUTH_REQUIRED)
         {
         ...
         //Page not found?
         else if(error == __ERROR_NOT_FOUND)
         {
...
error_t httpReadHeader(HttpConnection *connection)
{
   ...
   if(s != NULL)
   {
      ...
      *s = '\0';
      ...
      //Any error to report?
      if(error)
         return __ERROR_INVALID_REQUEST;
      ...
      while(*received < size)
      {
         ...
         if(connection->request.byteCount == 0)
         {
            ...
            if(!connection->request.byteCount)
            {
               ...
               return (*received > 0) ? __NO_ERROR : __ERROR_END_OF_STREAM;
            }
         }
...
error_t httpSendResponse(HttpConnection *connection, const char_t *uri)
{
   ...
   if(error) return __ERROR_NOT_FOUND;

   //Open the file for reading
   file = fsOpenFile(connection->buffer, FS_FILE_MODE_READ);
   //Failed to open the file?
   if(!file) return __ERROR_NOT_FOUND;
#else
   error_t error;
   size_t length;
   uint8_t *data;
   ...
   if(length == 0 && error == __ERROR_END_OF_STREAM)
   {
