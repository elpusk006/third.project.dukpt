...
const Ipv6Addr IPV6_LINK_LOCAL_ADDR_PREFIX =
   IPV6_ADDR(0xFF80, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000);
...
error_t ipv6StringToAddr(const char_t *str, Ipv6Addr *ipAddr)
{
   ...
   while(1)
   {
      ...
      if(isxdigit((uint8_t) *str))
      {
         ...
         if(value > 0xFFFF) return __ERROR_FAILURE;
      }
      //"::" symbol found?
      else if(!strncmp(str, "::", 2))
      {
         //The "::" can only appear once in an IPv6 address
         if(j >= 0) return __ERROR_FAILURE;
