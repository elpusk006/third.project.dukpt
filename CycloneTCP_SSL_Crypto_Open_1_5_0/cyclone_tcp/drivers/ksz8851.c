...
error_t ksz8851Init(NetInterface *interface)
{
   ...
   if(ksz8851ReadReg(interface, KSZ8851_REG_CIDER) != KSZ8851_REV_A2_ID)
      return __ERROR_WRONG_IDENTIFIER;
...
uint16_t ksz8851ReadReg(NetInterface *interface, uint8_t address)
{
...
#if 1
   //Set SPI mode 1
   interface->spiDriver->setMode(1);
   //Read first data bit
   data = (AT91C_BASE_PIOA->PIO_PDSR & AT91C_PA12_MISO) ? 0x0100 : 0x0000;
#endif
   ...
   data |= (interface->spiDriver->transfer(0x00) & 0xFE) << 7;
...
void ksz8851ReadFifo(NetInterface *interface, uint8_t *data, size_t length)
{
   ...
   interface->spiDriver->setMode(1);
   //Read first data bit
   value = (AT91C_BASE_PIOA->PIO_PDSR & AT91C_PA12_MISO) ? 1 : 0;

   //The first 4 bytes are dummy data and must be discarded
   for(i = 0; i < 4; i++)
      value = interface->spiDriver->transfer(0x00);
