...
error_t tlsIoWrite(TlsContext *context, const void *data, size_t length)
{
   ...
   //Check return value
   if(n != length) return __ERROR_WRITE_FAILED;
   ...
   //Any error to report?
   if(error) return __ERROR_WRITE_FAILED;
...
error_t tlsIoRead(TlsContext *context, void *data, size_t length)
{
   ...
   if(length != 0) return __ERROR_READ_FAILED;
   ...
   //Any error to report?
   if(error) return __ERROR_READ_FAILED;

   //Make sure that the requested number of bytes have been read
   if(n != length) return __ERROR_READ_FAILED;
